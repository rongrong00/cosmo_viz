...
#SBATCH -t 01:00:00
#SBATCH -p batch
#SBATCH -o output/sph_mw_f0_%j.out
#SBATCH -e output/sph_mw_f0_%j.err

# Mass-weighted metallicity + temperature maps for spiral-orbit frame 0.
module load hdf5
source ~/.bashrc
conda activate chii
...
OUTDIR=output/orbit_sph_spiral_mw_f0
...
srun -n 1 -c ${OMP_NUM_THREADS} --cpu-bind=threads \
    ...
    --camera   "$CAMERA" \
    --output   "$OUTDIR" \
    --fields   gas_density,metallicity,temperature \
    --no-volume
