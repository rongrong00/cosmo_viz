...
#SBATCH -t 00:30:00
#SBATCH -p batch
#SBATCH -o output/gasonly_%j.out
#SBATCH -e output/gasonly_%j.err
...
srun -n 10 ./build/gridder \
    ...
    --config config/gasonly_grid.yaml \
    --output output/grids/

echo "=== RENDERER ==="
OMP_NUM_THREADS=32 ./build/renderer \
    --grid output/grids/grid_gasonly_snap121.hdf5 \
    --camera config/gasonly_camera.yaml \
