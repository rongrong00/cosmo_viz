...
#SBATCH -t 00:30:00
#SBATCH -p batch
#SBATCH -o output/allgas_%j.out
#SBATCH -e output/allgas_%j.err
...
srun -n 10 ./build/gridder \
    ...
    --config config/allgas_grid.yaml \
    --output output/grids/

echo "=== RENDERER ==="
OMP_NUM_THREADS=32 ./build/renderer \
    --grid output/grids/grid_allgas_snap121.hdf5 \
    --camera config/allgas_camera.yaml \
